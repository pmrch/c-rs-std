...
#define TEST_VEC_PUSH_STRESS_INT(c_type, rust_type, num_iter, type_max)        \
    do {                                                                       \
        Vec vec = {0};                                                         \
        Result res = vec_new(&vec, rust_type);                                 \
        int ok_new = !res.err && vec.data && vec.magic == VEC_MAGIC_INIT &&    \
                     vec.capacity > 0;                                         \
        if (ok_new == 0) {                                                     \
            return -1;                                                         \
        }                                                                      \
        ...
        for (size_t i = 0; i < (size_t)num_iter; i++) {                        \
            ...
            Result res_p = vec_push(&vec, &pushed);                            \
                                                                               \
            int ok_p = !res_p.err && vec.data &&                               \
                       vec.magic == VEC_MAGIC_INIT && vec.capacity > 0;        \
...
#define TEST_VEC_PUSH_STRESS_FLOAT(c_type, rust_type, num_iter)               \
    do {                                                                      \
        Vec vec = {0};                                                        \
        Result res = vec_new(&vec, rust_type);                                \
        int ok_new = !res.err && vec.data && vec.magic == VEC_MAGIC_INIT &&   \
...
inline static int test_vec_new(RustType type) {
    ...
    int ok =
        !res.err && vec.data && vec.magic == VEC_MAGIC_INIT && vec.capacity > 0;
    vec_free(&vec);
    return ok ? 0 : -1;
}
...
#define X(type_enum, ctype, maxval)                            \
    case type_enum:                                            \
        TEST_VEC_PUSH_STRESS_INT(ctype, type_enum, 1, maxval); \
        break;
        RUSTTYPE_MAP_INT
#undef X
...
#define X(type_enum, ctype)                              \
    case type_enum:                                      \
        TEST_VEC_PUSH_STRESS_FLOAT(ctype, type_enum, 1); \
        break;
        RUSTTYPE_MAP_FLOAT
#undef X
...
#define X(type_enum, ctype, maxval)                               \
    case type_enum:                                               \
        TEST_VEC_PUSH_STRESS_INT(ctype, type_enum, 1000, maxval); \
        break;

        // 2. RUSTTYPE_MAP_INT expands, substituting X for each entry
        RUSTTYPE_MAP_INT
/* becomes:
 * case TYPE_U8:  TEST_VEC_PUSH_STRESS_INT(uint8_t,  TYPE_U8,  1000, 255);
 * break; case TYPE_U16: TEST_VEC_PUSH_STRESS_INT(uint16_t, TYPE_U16, 1000,
 * UINT16_MAX); break;
 * ... etc
 */
...
#define X(type_enum, ctype)                                 \
    case type_enum:                                         \
        TEST_VEC_PUSH_STRESS_FLOAT(ctype, type_enum, 1000); \
        break;
...
#define X(type_enum, ctype, maxval)                               \
    case type_enum:                                               \
        TEST_VEC_PUSH_STRESS_INT(ctype, type_enum, 14e7, maxval); \
        break;
...
#define X(type_enum, ctype)                                 \
    case type_enum:                                         \
        TEST_VEC_PUSH_STRESS_FLOAT(ctype, type_enum, 14e7); \
        break;
