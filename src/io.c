...
int get_filesize(const char* filename) {
    ...
    if (stat(filename, &st) == 0) {
        LOG_DEBUG("get_filesize: %s = %lld bytes", filename,
...
Result file_close(File* in) {
    if (!in || !in->handle || in->closed) {
        LOG_ERROR(
            "file_close error: passed argument file was uninitialized or "
            "already closed");
        return RESULT_ERR(ERR_IO);
    }
...
Result read_to_string(const char* path, char** out_buffer) {
    if (!out_buffer) {
        LOG_ERROR(
            "read_to_string error: output buffer pointer was not initialized");
    ...
    *out_buffer = NULL;
    ...
    if (size < 0) {
        LOG_ERROR("read_to_string error: file size was negative for '%s'",
                  path);
        file_close(&file);
        return RESULT_ERR(ERR_IO);
    }
    ...
    if (!buffer) {
        LOG_ERROR(
            "read_to_string error: failed to allocate buffer for file '%s'",
            path);
        file_close(&file);
    ...
    if (read != (size_t)size) {
        if (feof(file.handle))
            LOG_ERROR(
                "read_to_string: unexpected EOF while reading %d bytes from "
                "'%s'",
                size, path);
        else if (ferror(file.handle))
            LOG_ERROR(
                "read_to_string: IO error while reading %d bytes from '%s'",
                size, path);
        else
            LOG_ERROR(
                "read_to_string: unknown read error (%zu/%d bytes read) from "
                "'%s'",
                read, size, path);
    ...
    *out_buffer = buffer;
    ...
    LOG_INFO("read_to_string: successfully read %d bytes from '%s'", size,
             path);
    file_close(&file);
    return RESULT_OK();
}
