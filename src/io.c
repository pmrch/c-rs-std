...
Result read_to_string(const char *path, char **out_buffer) {
    ...
    *out_buffer = NULL;
    ...
    if (read != (size_t)size) {
        if (feof(file.handle))
            LOG_ERROR("read_to_string: unexpected EOF while reading %zu bytes from '%s'", size, path);
        else if (ferror(file.handle))
            LOG_ERROR("read_to_string: IO error while reading %zu bytes from '%s'", size, path);
        else
            LOG_ERROR("read_to_string: unknown read error (%zu/%zu bytes read) from '%s'", read, size, path);
    ...
    *out_buffer = buffer;
    ...
    LOG_INFO("read_to_string: successfully read %lld bytes from '%s'", size, path);
    file_close(&file);
    return RESULT_OK();
}
